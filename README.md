...
1. Run `rake db:seed`.  When it is finished, it will tell you how long the process took (in seconds).  Record the amount of time. **4328.02 seconds**
...
1. Open Chrome's timeline in developer tools, then hit Cmd-R on your keyboard.  The timeline will track time to load the page.  Record the following:
  ...
  1. The time given by Rails at the top of the page:  **1391.43 s**
  1. The time given by Rails at the bottom of the page (sorry for the long scroll):  **1392.59 s**
...
1. Record how long it takes to run the migrations that add indices.  **2.58 s**
1. Reload the root page and record the four time numbers again.  
  1. Total time in Chrome's timeline:  **21.64 s**  
  1. "Idle" time in Chrome's timeline:  **10.48 s**
  1. The time given by Rails at the top of the page:  **6.76 s**
  1. The time given by Rails at the bottom of the page (sorry for the long scroll):  **8.09 s**
Calculate your percent improvement in runtime.  **99.5% shorter runtime**
...
1. Once you have optimized your code as much as you think you can, drop the database, run `rake db:migrate`, and then time how long it takes to run `rake db:seed`.  **3127.51 s**  
Was there an improvement or a worsening of runtime?  **Improvement**  
...
* Record the size of your database (in bytes).  **425 mb**
* Record the size of your development log.  **???**
* Give at least one method (feel free to Google) for reducing the size of one of these, yet keeping your data intact.  **config.legger**
* Do you think that this is smaller, about right, or larger than the size of databases you'll be working with in your career?  **smaller**
